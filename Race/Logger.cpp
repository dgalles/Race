...
void Logger::sendProfileData(std::string data)
{
	...
	sprintf_s(buffer, 10000, "SetProfile;Race;%s\n",data.c_str());
...
std::string Logger::getProfileData()
{
	...
	sslWrite(secureConnection,"GetProfile;Race\n");
	...
	Logger::StartSession(Ogre::String header)
{
	...
	if (header.length() > 0)
	{
		sprintf_s(mSendBuffer, DEFAULT_BUFSIZE, "insert;{\"game\":\"Race\", \"patient_id\":\"%s\",\"timestep\":%f,\"timestamp\":\"%s\", %s}\n", 
			mCurrentUsername.c_str(), mTimeStep, mBegBuf,header.c_str());

	}
	else
	{
		sprintf_s(mSendBuffer, DEFAULT_BUFSIZE, "insert;{\"game\":\"Race\", \"patient_id\":\"%s\",\"timestep\":%f,\"timestamp\":\"%s\"}\n", 
...
int Logger::LogAndSend(const char *attribName, float value, int time)
{
	...
	sprintf_s(mSendBuffer, 
		DEFAULT_BUFSIZE,
		"set;{\"game\":\"Race\", \"patient_id\":\"%s\", \"timestamp\": \"%s\"};{\"%s.%d\":%f}\n",
...
int Logger::LogAndSend(const char* attribName, float value[], int size, int time)
{
	...
	sprintf_s(mSendBuffer, 
		DEFAULT_BUFSIZE,
		"set;{\"game\":\"Race\", \"patient_id\":\"%s\", \"timestamp\": \"%s\"};{\"%s.%d\":[%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f]}\n",
...
int Logger::LogAndSend(const char *attribName, int value, int time)
{
	...
	sprintf_s(mSendBuffer, 
		DEFAULT_BUFSIZE,
		"set;{\"game\":\"Race\", \"patient_id\":\"%s\", \"timestamp\": \"%s\"};{\"%s.%d\":%d}\n",
		mCurrentUsername.c_str(), mBegBuf, attribName, time, value);
