...
void Ghost::updateGhost(float time)
{
	if (mAlive)
	{
		...
		{
			mGameObject->pitch(Ogre::Radian(Ogre::Degree(-lean2)));
			mGameObject->setPosition(pos  + up *( - Ogre::Math::Sin(Ogre::Degree(lean2)) * mGameObject->minPointLocalScaled().z* 0.8f) +0);
		}
		...
		if (lean2 <  0 &&  mGhostInfo->mLeanEqualsDuck)
		{
			...
			mGameObject->setScale(Ogre::Vector3(5,diff,10));
		}


		mGameObject->roll(Ogre::Radian(Ogre::Degree(lean)));
...
void  Ghost::startPlayback()
{
	mDataindex = 0;
	mCurrentTime = 0;
	mGameObject = new GameObject(GameObject::ObjectType::PLAYER);
	mGameObject->loadModel("car.mesh", mWorld->SceneManager());
	mGameObject->setScale(Ogre::Vector3(5,6,10));
	mGameObject->setMaterial("ghostMaterial"); 

	//mGameObject->setAlpha(0.0);
	//mGameObject->setAlpha(0.3f);
	mPlayingBack = true;
	mRecording = false;
	mAlive = true;
}
